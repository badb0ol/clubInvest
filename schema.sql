...
create table clubs (
  ...
  tax_liability decimal(15, 2) default 0.00, -- Provision pour Flat Tax (30%)
  linked_bank text, -- Nom de la banque connectée (ex: "Trade Republic")
...
create table transactions (
  ...
  type text not null check (type in ('DEPOSIT', 'WITHDRAWAL', 'BUY', 'SELL', 'SNAPSHOT')),
...
-- NOTE: Full policies are defined in rls_policies.sql
-- Run rls_policies.sql after this schema to enable proper RLS.
...
-- =====================================================
-- alter table clubs add column if not exists tax_liability decimal(15, 2) default 0.00;
-- alter table clubs add column if not exists linked_bank text;
-- alter table transactions drop constraint if exists transactions_type_check;
-- alter table transactions add constraint transactions_type_check
--   check (type in ('DEPOSIT', 'WITHDRAWAL', 'BUY', 'SELL', 'SNAPSHOT'));